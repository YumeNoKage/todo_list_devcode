        ...
        <div class="col-12 mt-5 pt-3" v-else-if="data.length > 0 && !isLoading">
            <div class="row">
                <div class="col-lg-3 col-md-4 col-sm-12 mb-4 position-relative text-decoration-none" v-for="(item, index) in data" :key="index">
                    ...
                    <button class="p-0 border-0 bg-transparent position-absolute bottom-0 end-0 pe-1 mx-4 mb-3" data-bs-target="#deleteActivity" data-bs-toggle="modal" @click="$router.push('/'); idActivity = item.id, activityName = item.title, deleteAction = true" data-cy="activity-item-delete-button">
